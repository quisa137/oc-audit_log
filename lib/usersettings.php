...
//				$settings[] = Data::TYPE_STORAGE_FAILURE;
	...
	public static function filterUsersBySetting($users, $method, $type) {
		...
		$potentialUsers = $preferences->getValueForUsers('audit_log', 'notify_' . $method . '_' . $type, $users);
		...
		if ($method == 'email') {
			$potentialUsers = $preferences->getValueForUsers('audit_log', 'notify_setting_batchtime', array_keys($filteredUsers));
			foreach ($potentialUsers as $user => $value) {
