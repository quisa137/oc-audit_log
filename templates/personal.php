...
<form id="activity_notifications" class="section">
	...
	<table class="grid activitysettings">
		...
		<?php foreach ($_['activities'] as $activity => $data): ?>
			<tr>
				<td class="small"><label for="<?php p($activity) ?>_email"> <input
						type="checkbox" id="<?php p($activity) ?>_email"
						name="<?php p($activity) ?>_email" value="1"
						class="<?php p($activity) ?> email" <?php if ($data['email']): ?>
						checked="checked" <?php endif; ?> />
				</label></td>
				<td class="small"><label for="<?php p($activity) ?>_stream"> <input
						type="checkbox" id="<?php p($activity) ?>_stream"
						name="<?php p($activity) ?>_stream" value="1"
						class="<?php p($activity) ?> stream"
	...
	<br /> <input id="notify_setting_self" name="notify_setting_self"
		...
		checked="checked" <?php endif; ?> /> <label for="notify_setting_self"><?php p($l->t('Notify about my own actions')); ?></label>
	<br />

	<?php if (empty($_['activity_email'])): ?>
		<br /> <strong><?php p($l->t('You need to set up your email address before you can receive notification emails.')); ?></strong>
	<?php endif; ?>
	...
	<select id="notify_setting_batchtime" name="notify_setting_batchtime">
		<option value="0"
			<?php if ($_['setting_batchtime'] === \OCA\Audit_log\UserSettings::EMAIL_SEND_HOURLY): ?>
			selected="selected" <?php endif; ?>><?php p($l->t('Hourly')); ?></option>
		<option value="1"
			<?php if ($_['setting_batchtime'] === \OCA\Audit_log\UserSettings::EMAIL_SEND_DAILY): ?>
			selected="selected" <?php endif; ?>><?php p($l->t('Daily')); ?></option>
		<option value="2"
			<?php if ($_['setting_batchtime'] === \OCA\Audit_log\UserSettings::EMAIL_SEND_WEEKLY): ?>
